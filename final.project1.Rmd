...
``` {r 3 Methods}
sba_merged_model <- droplevels(sba_merged_final)
...
model_CART <- function(split, formula, ...) {
  ...
  predictions_CART <- bind_cols(
   SBA_test,
    decision_tree = predict(model, SBA_test)
)
...
model_linear <- function(split, formula, ...) {
  ...
  analysis_data <- analysis_split
  assessment_data <- assessment_split
  model <-
    linear_reg() %>%
    set_engine("lm") %>%
    fit(formula,
  ...
  rmse <- bind_cols(
    ...
    rmse(truth = Approved_Loan_Content, estimate = .pred) %>%
    pull(.estimate)
...
SBA_resamples %>%
  mutate(
    rmse_CART1 = map_dbl(splits,
                        .f = model_CART(split = .x,
                                        formula = `Approved_Amount_Content` ~ `Verified_Loss_Content` + payann + emp + rcptot)),
    rmse_linear1 = map_dbl(.x = splits,
                        .f = model_linear(split = .x,
                                        formula = `Approved_Amount_Content` ~ `Verified_Loss_Content` + payann + emp + rcptot))) %>%
      select(id, rmse_CART1, rmse_linear1)
