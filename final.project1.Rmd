...
>>>>>>> 33be186bffe3c7741293d2f2471dc4644075319b
loan_missing <- sba_merged_final %>%
  filter(is.na(`Approved_Amount_Content`))
loan_nonmissing <-sba_merged_final %>%
<<<<<<< HEAD
  filter(!is.na(`Verified Loss Content`))

split <- initial_split(data = sba_merged_final prop = 0.8)
=======
  filter(!is.na(`Approved_Amount_Content`))
split <- initial_split(data = sba_merged_final, prop = 0.8)
>>>>>>> 33be186bffe3c7741293d2f2471dc4644075319b
missing_train <- training(split)
missing_test <- testing(split)

model <-
  decision_tree(mode = "regression") %>%
  set_engine("rpart") %>%
  fit(`Approved_Amount_Content` ~ `Verified_Loss_Content` + payann + emp + rcptot + covid_cases + covid_deaths,
      data = missing_train)

bind_cols(
  missing_test,
  predict(model, missing_test)
) %>%
  metrics(truth = `Approved_Amount_Content`,
          estimate = .pred)

loan_missing <- loan_missing %>%
  select(-`Approved_Amount_Content`) %>%
  bind_cols(
    VAR = predict(model, loan_missing)
  ) %>%
  rename(price = .pred)
...
- avg_county_approved_content_loan ~ avg_county_verified_content_loan + emp + payann + rcptot + payqrt1 + estab + covid_cases + covid_deaths + MBay_effect

<<<<<<< HEAD
- total_county_approved_content_loan ~ total_county_verified_content_loan + emp + payann + rcptot + payqrt1 + estab + covid_cases + covid_deaths + MBay_effect

``` {r setting seed and splitting into training and testing}
=======
``` {r 3 Methods for Approved Content Loan Size }
sba_merged_model <- SBA_merged_imputed %>%
  select(Approved_Amount_Content, Verified_Loss_Content, payann, payqrt1, emp, estab, rcptot, MBay_effect, covid_cases, covid_deaths)
...
```{r RandomForest algorithm}
#https://www.r-bloggers.com/how-to-implement-random-forests-in-r/

#SBA_RF <- randomForest(`Approved_Amount_Content` ~ ., data = SBA_merged_imputed, importance = TRUE)
...
```{r CART algorithm}
#SBA_CART is function in other r-script:

SBA_CART(split = , formula = , dep_var = , indep_var = )
```

```{r linear regression algorithm}
#SBA_linear is function in other r-script:

SBA_linear(split = , formula = , dep_var = , indep_var = )
=======
model_CART <- function(split, formula, ...) {
  ...
  rmse <- bind_cols(
    ...
    rmse(truth = Approved_Amount_Content, estimate = .pred) %>%
    pull(.estimate)
...
model_linear <- function(split, formula, ...) {
...
SBA_resamples %>%
  mutate(
    rmse_CART1 = map_dbl(splits,
                        .f = ~model_CART(.x,
                                        formula = Approved_Amount_Content ~ `Verified_Loss_Content` + emp + estab + rcptot + MBay_effect)),
    rmse_linear1 = map_dbl(.x = splits,
                        .f = ~model_linear(.x,
                                        formula = Approved_Amount_Content ~ `Verified_Loss_Content` + emp + estab + rcptot + MBay_effect)),
  rmse_CART2 = map_dbl(splits,
                        .f = ~model_CART(.x,
                                        formula = Approved_Amount_Content ~ `Verified_Loss_Content` + emp + estab + rcptot + covid_deaths + MBay_effect)),
    rmse_linear2 = map_dbl(.x = splits,
                        .f = ~model_linear(.x,
                                        formula = Approved_Amount_Content ~ `Verified_Loss_Content` + emp + estab + rcptot + covid_deaths + MBay_effect)),
  rmse_CART3 = map_dbl(splits,
                        .f = ~model_CART(.x,
                                        formula = Approved_Amount_Content ~ `Verified_Loss_Content` + emp + MBay_effect)),
    rmse_linear3 = map_dbl(.x = splits,
                        .f = ~model_linear(.x,
                                        formula = Approved_Amount_Content ~ `Verified_Loss_Content` + emp + MBay_effect))) %>%
      select(id, rmse_CART1, rmse_linear1, rmse_CART2, rmse_linear2, rmse_CART3, rmse_linear3)
...
sba_merged_model <- sba_merged_final %>%
  select(Approved_Amount_Content, Verified_Loss_Content, payann, payqrt1, emp, estab, rcptot, MBay_effect, covid_cases, covid_deaths, total_county_approved_content_loan, Total_Verified_Loss)
...
SBA_RF <- randomForest(`Approved_Amount_Content` ~ ., data = na.exclude(sba_merged_model), importance = TRUE)
...
model_CART <- function(split, formula, ...) {
  ...
  rmse <- bind_cols(
    ...
    rmse(truth = total_county_approved_content_loan, estimate = .pred) %>%
    pull(.estimate)
...
SBA_resamples %>%
  mutate(
    rmse_CART1 = map_dbl(splits,
                        .f = ~model_CART(.x,
                                        formula = total_county_approved_content_loan ~ Total_Verified_Loss + emp + estab + rcptot + MBay_effect)),
    rmse_linear1 = map_dbl(.x = splits,
                        .f = ~model_linear(.x,
                                        formula = total_county_approved_content_loan ~ Total_Verified_Loss + emp + estab + rcptot + MBay_effect)),
  rmse_CART2 = map_dbl(splits,
                        .f = ~model_CART(.x,
                                        formula = total_county_approved_content_loan ~ Total_Verified_Loss + emp + estab + rcptot + covid_deaths + MBay_effect)),
    rmse_linear2 = map_dbl(.x = splits,
                        .f = ~model_linear(.x,
                                        formula = total_county_approved_content_loan ~ Total_Verified_Loss + emp + estab + rcptot + covid_deaths + MBay_effect)),
  rmse_CART3 = map_dbl(splits,
                        .f = ~model_CART(.x,
                                        formula = Approved_Amount_Content ~ Total_Verified_Loss + emp + MBay_effect)),
    rmse_linear3 = map_dbl(.x = splits,
                        .f = ~model_linear(.x,
                                        formula = Approved_Amount_Content ~ Total_Verified_Loss + emp + MBay_effect))) %>%
      select(id, rmse_CART1, rmse_linear1, rmse_CART2, rmse_linear2, rmse_CART3, rmse_linear3)
