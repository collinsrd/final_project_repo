...
#pre-process:
sba_merged_model_rf <- SBA_merged_imputed %>%
...
sba_merged_model_rf <- na.omit(sba_merged_model_rf)
...
sba_merged_model <- na.omit(sba_merged_model) %>%
  sample_n(50000) 
...
(default_rmse <- sqrt(sba_rf$prediction.error))
```

```{r RandomForest algorithm for total_county_eidl_loan}
n_features <- length(setdiff(names(SBA_train_rf), "total_county_eidl_loan"))
...
final_model <- decision_tree(mode = "regression") %>%
    ...
    fit(formula = avg_county_eidl_loan ~ total_county_verified_content_loan + 
                                          emp + payann + rcptot + payqrt1 + estab + covid_cases + 
                                          covid_deaths + MBay_effect, , data = SBA_train)

  
rmse <- bind_cols(
  bake(final_recipe, new_data = SBA_train),
  pred = predict(final_model, bake(final_recipe, new_data = SBA_train)) %>%
